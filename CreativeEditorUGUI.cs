...
namespace CloudMeadow.CreativeMode
{
    internal sealed class CreativeEditorUGUI : MonoBehaviour
    {
        ...
        private bool _showBloodlineFilter;
        ...
        private void Update()
        {
            ...
            if (_cursorCanvas != null && _cursorCanvas.gameObject.activeSelf && _cursorRect != null)
            {
                _cursorRect.anchoredPosition = Input.mousePosition;
            }
        ...
        private void SetVisible(bool value)
        {
            ...
            if (_cursorCanvas != null) _cursorCanvas.gameObject.SetActive(value);
            if (value)
            {
                Cursor.visible = false;
                Cursor.lockState = CursorLockMode.None;
                // Set yellow square cursor
                if (_cursorRect != null)
                {
                    var img = _cursorRect.GetComponent<Image>();
                    if (img != null)
                    {
                        // Create yellow square texture
                        var tex = new Texture2D(16, 16, TextureFormat.RGBA32, false);
                        var pixels = new Color[16 * 16];
                        for (int i = 0; i < pixels.Length; i++) pixels[i] = new Color(1f, 0.92f, 0.2f, 1f);
                        tex.SetPixels(pixels);
                        tex.Apply();
                        img.sprite = Sprite.Create(tex, new Rect(0, 0, 16, 16), new Vector2(0, 0));
                        _cursorRect.sizeDelta = new Vector2(16, 16);
                    }
                }
                RefreshModule();
            }
        ...
        private void Build()
        {
            ...
            SetRect(sidebar, Vector2.zero, new Vector2(0, 1), new Vector2(0, 0), new Vector2(160, -68));
            var layout = sidebar.gameObject.AddComponent<VerticalLayoutGroup>();
            layout.padding = new RectOffset(6, 6, 6, 6); layout.spacing = 2; layout.childControlHeight = true; layout.childForceExpandWidth = true; layout.childForceExpandHeight = false;
            for (int i = 0; i < Modules.Length; i++)
            {
                string captured = Modules[i];
                var b = MakeButton(captured, sidebar, delegate { _module = captured; RefreshModule(); });
                b.GetComponent<LayoutElement>().preferredHeight = 22;
                _sidebarButtons[captured] = b;
            }

            _content = Panel("Content", window, new Color(0.035f, 0.05f, 0.07f, 0.25f));
            SetRect(_content, Vector2.zero, Vector2.one, new Vector2(160, 0), new Vector2(0, -68));
            ...
            // Create cursor layer ABOVE the menu (sortingOrder > 100)
            var cursorRoot = new GameObject("CreativeCursorCanvas", typeof(RectTransform), typeof(Canvas), typeof(CanvasScaler));
            cursorRoot.transform.SetParent(transform, false);
            _cursorCanvas = cursorRoot.GetComponent<Canvas>();
            _cursorCanvas.renderMode = RenderMode.ScreenSpaceOverlay;
            _cursorCanvas.sortingOrder = 32767;
            var cursorScaler = cursorRoot.GetComponent<CanvasScaler>();
            cursorScaler.uiScaleMode = CanvasScaler.ScaleMode.ConstantPixelSize;
            cursorScaler.scaleFactor = 1f;

            var cursorObj = new GameObject("Cursor", typeof(RectTransform), typeof(Image));
            cursorObj.transform.SetParent(cursorRoot.transform, false);
            ...
            _cursorRect.sizeDelta = new Vector2(16, 16);
            var cursorImg = cursorObj.GetComponent<Image>();
            cursorImg.color = Color.white;
            cursorImg.raycastTarget = false;
        }

        private Sprite LoadCursorSprite()
        {
            // Return null to use system cursor instead of custom cursor
            return null;
        }

        private Sprite CreateHandCursor()
        {
            // Create a larger hand/pointer cursor (48x48)
            var tex = new Texture2D(48, 48, TextureFormat.RGBA32, false);
            var pixels = new Color[48 * 48];
            for (int i = 0; i < pixels.Length; i++) pixels[i] = Color.clear;
            
            // Draw a simple hand pointing up-right (like Windows hand cursor)
            // Palm
            for (int y = 15; y < 38; y++)
                for (int x = 8; x < 30; x++)
                    pixels[y * 48 + x] = new Color(0.9f, 0.7f, 0.5f, 1f);
            // Fingers
            for (int y = 8; y < 22; y++)
                for (int x = 22; x < 33; x++)
                    pixels[y * 48 + x] = new Color(0.9f, 0.7f, 0.5f, 1f);
            ...
            tex.SetPixels(pixels);
            tex.Apply();
            return Sprite.Create(tex, new Rect(0, 0, 48, 48), new Vector2(0, 0));
        }
            ...
            else if (_module == "Monsters")
            {
                ...
                if (_selectedMonster != null)
                {
                    ...
                    for (int ti = 0; ti < traits.Length; ti++)
                    {
                        ...
                        string quality = GameApi.GetTraitQuality(trait);
                        string bloodlineTag = GameApi.IsBloodlineTrait(trait) ? " [Bloodline]" : string.Empty;
                        AddLine(monsterInspector, GameApi.GetTraitDisplayName(trait) + "  [" + quality + bloodlineTag + "]");
                    ...
                    {
                        AddAction(monsterInspector, _showBloodlineFilter ? "▼ Show all traits" : "▶ Show only Bloodline traits", delegate { _showBloodlineFilter = !_showBloodlineFilter; _traitPage = 0; RefreshModule(); });
                        // Filter traits if bloodline filter is active
                        var filteredTraits = new System.Collections.Generic.List<object>();
                        for (int ati = 0; ati < availableTraits.Length; ati++)
                        {
                            object traitDef = availableTraits[ati];
                            if (traitDef == null || GameApi.MonsterHasTrait(_selectedMonster, traitDef)) continue;
                            if (_showBloodlineFilter && !GameApi.IsBloodlineTrait(traitDef)) continue;
                            filteredTraits.Add(traitDef);
                        }
                        var filteredArray = GameApi.SortTraitsByPriority(filteredTraits.ToArray());
                        AddLine(monsterInspector, "Trait catalog page " + (_traitPage + 1) + " / " + Mathf.Max(1, Mathf.CeilToInt(filteredArray.Length / 18f)));
                        AddAction(monsterInspector, "Previous trait page", delegate { if (_traitPage > 0) _traitPage--; RefreshModule(); });
                        AddAction(monsterInspector, "Next trait page", delegate { if ((_traitPage + 1) * 18 < filteredArray.Length) _traitPage++; RefreshModule(); });
                        ...
                        Transform traitGrid = gridObject.transform;
                        int filteredStart = _traitPage * 18, filteredLimit = Mathf.Min(filteredArray.Length, filteredStart + 18);
                        for (int ati = filteredStart; ati < filteredLimit; ati++)
                        {
                            object traitDef = filteredArray[ati];
                            object capturedDef = traitDef; var addTraitMonster = _selectedMonster;
                            string quality = GameApi.GetTraitQuality(traitDef);
                            string bloodlineTag = GameApi.IsBloodlineTrait(traitDef) ? " [Bloodline]" : string.Empty;
                            AddTraitAction(traitGrid, GameApi.GetTraitDisplayName(traitDef), GameApi.GetTraitDescription(traitDef), quality + bloodlineTag, delegate { GameApi.AddTraitToMonster(addTraitMonster, capturedDef, 1); RefreshModule(); });
                        }
                    }
                }
                else
